...
private:
    ...
    struct ListNode {
        ...
        template <typename... Args>
        ListNode(Alloc alloc, Args&&... args) {
            traits_t::construct(alloc, &key_value, std::forward<Args>(args)...);
        ...
        ListNode(NodeType&& node)
            : key_value(std::move(const_cast<Key&>(node.first)),
                        std::move(node.second))
            , hash(Hash{}(key_value.first))
        {}
        
        ListNode(ListNode&& other)
            : key_value(std::move(const_cast<Key&>(other.key_value.first)),
                        std::move(other.key_value.second))
            , hash(other.hash)
        {}

        ListNode(const ListNode& other)
            : key_value(other.key_value.first, other.key_value.second)
            , hash(other.hash)
        {}
...
public:
    ...
    std::pair<Iterator, bool> emplace(Args&&... args) {
        ...
        ListNode list_node(std::move(*node));
        size_t index = list_node.hash % iterators_.size();
