...
private:
    ...
    void sub_abs(const BigInteger& number) {
        bool smaller = less_abs(number);
        const std::vector<int>& less = smaller ? buffer_ : number.buffer_;
        const std::vector<int>& greater = smaller ? number.buffer_ : buffer_;
        int carry = 0;
        for (size_t i = 0; i < number.buffer_.size() || carry > 0; ++i) {
...
template <int N, int K, bool less>
struct IsPrimeHelper {
    static const std::conditional<N % K != 0,
                                  decltype(IsPrimeHelper<N, K + 1, K * K <= N>::value),
...
private:
    ...
    Residue inverse() const {
        Residue result = 1, tmp = value_;
        for (size_t power = N - 2; power != 0; power /= 2, tmp *= tmp)
            if (power % 2 == 1)
                result *= tmp;
        return result;
    }
...
public:
    ...
    Residue(int source) : value_((source % static_cast<int>(N) + N) % N) {}
    ...
    Residue& operator+=(const Residue& number) {
    ...
    Residue& operator-=(const Residue& number) {
        value_ = N + value_ - number.value_;
        value_ = value_ < N ? value_ : value_ - N;
        return *this;
    }
    
    Residue& operator*=(const Residue& number) {
        value_ = value_ * number.value_ % N;
        return *this;
    }
    
    Residue& operator/=(const Residue& number) {
        static_assert(IsPrime<N>::value, "Modulus should be prime");
        return *this *= number.inverse();
    }
};

template <size_t N>
Residue<N> operator+(Residue<N> left, const Residue<N>& right) {
    return left += right;
}
...
class Matrix {
private:
    std::vector<std::vector<Field> > table_;
    
    Field gaussian_elimination(size_t columns) {
        Field det = 1;
        for (size_t column = 0, row = 0; column < columns && row < M; ++column) {
            size_t nonzero_cell = row;
            for (size_t i = row; i < M; ++i) {
                if (table_[i][column] != 0) {
                    nonzero_cell = i;
                    break;
                }
            }
            if (table_[nonzero_cell][column] == 0) {
                det = 0;
                continue;
            }
            table_[row].swap(table_[nonzero_cell]);
            if (row != nonzero_cell)
                det *= -1;
            det *= table_[row][column];
            Field pivot = table_[row][column];
            for (size_t i = column; i < N; ++i)
                table_[row][i] /= pivot;
            for (size_t i = 0; i < M; ++i) {
                if (i != row) {
                    Field current = table_[i][column];
                    for (size_t j = column; j < N; ++j) {
                        table_[i][j] -= table_[row][j] * current;
                    }
                }
            }
            ++row;
...
public:
    Matrix() : table_(M, std::vector<Field>(N)) {
        if (M != N)
            return;
        for (size_t i = 0; i < M; ++i)
            table_[i][i] = 1;
    ...
    Matrix(const std::vector<std::vector<Field> >& init) : table_(N, std::vector<Field>(M)) {
        for (size_t i = 0; i < M; ++i) {
    ...
    Matrix(const std::vector<std::vector<int> >& init) : table_(N, std::vector<Field>(M)) {
        for (size_t i = 0; i < M; ++i) {
            for (size_t j = 0; j < N; ++j) {
                table_[i][j] = init[i][j];
            }
    ...
    Field det() const {
        static_assert(M == N, "Determinant is available only for quadratic matricies");
        Matrix tmp = *this;
        return tmp.gaussian_elimination(M);
    }
    
    size_t rank() const {
        Matrix tmp = *this;
        tmp.gaussian_elimination(N);
        size_t row;
        for (row = 0; row < M; ++row) {
            bool empty_row = true;
    ...
    Field trace() const {
        static_assert(M == N, "Trace is available only for quadratic matricies");
        Field result;
        for (size_t i = 0; i < M; ++i) {
            result += table_[i][i];
        }
    ...
    void invert() {
        static_assert(M == N, "Invert is available only for quadratic matricies");
        Matrix<M, M * 2, Field> tmp;
        for (size_t i = 0; i < M; ++i) {
            for (size_t j = 0; j < M; ++j) {
                tmp[i][j] = table_[i][j];
                tmp[i][M + j] = i != j ? 0 : 1;
        ...
        tmp.gaussian_elimination(M);
        for (size_t i = 0; i < M; ++i) {
            for (size_t j = 0; j < M; ++j) {
                table_[i][j] = tmp[i][M + j];
            }
        }
    }
    ...
    bool operator==(const Matrix& matrix) const {
        for (size_t i = 0; i < M; ++i)
            for (size_t j = 0; j < N; ++j)
                if (table_[i][j] != matrix.table_[i][j])
                    return false;
        return true;
    }
...
template <size_t M, size_t N, typename Field = Rational>
Matrix<M, N, Field> operator+(Matrix<M, N, Field> left, const Matrix<M, N, Field>& right) {
    return left += right;
}
...
template <size_t M, size_t N, typename Field = Rational>
Matrix<M, N, Field> operator*(const Field& left, Matrix<M, N, Field> right) {
    return right *= left;
}
...
Matrix<N, N, Field>& operator*=(Matrix<N, N, Field>& left, const Matrix<N, N, Field>& right) {
