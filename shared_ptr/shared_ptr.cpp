...
struct ControlBlockDirect : BaseControlBlock {
    ...
    ControlBlockDirect(T* initPointer, Deleter initDeleter, Alloc&& initAlloc)
        : pointer(initPointer), deleter(initDeleter), alloc(std::move(initAlloc))
    {}
    ...
    void deallocateBlock() override {
        std::allocator_traits<BlockAlloc>::deallocate(alloc, this, 1);
    }
};
...
struct ControlBlockAllocateShared : BaseControlBlock {
    ...
    ControlBlockAllocateShared(Alloc&& initAlloc, Args&&... args)
        : object(std::forward<Args>(args)...), alloc(std::move(initAlloc))
    {}
...
public:
    ...
    //SharedPtr(SharedPtr&& other) : data_(other.data_) {
        //other.data_ = nullptr;
    //}
...
public:
    ...
    //WeakPtr(const SharedPtr<T>& other) : data_(other.data_) {
        //increment();
    //}
    ...
    //WeakPtr(WeakPtr&& other) : data_(other.data_) {
        //other.data_ = nullptr;
    //}
